...
\App\User::where('id', 17)->value('age');  // 返回 17
\App\User::find(3);                        // 找出主键等于 3 的记录
\App\User::find([1, 2]);                        // 找出主键等于 1 和 2 的记录
\App\User::pluck('age');                   // 返回包含字段值的集合
\App\User::pluck('age', 'id');             // 返回关联集合 id => age，最多 2 个参数
\App\User::count();                        // 返回数字
\App\User::max('id');                      // 返回数字或 null
\App\User::min('id');
\App\User::avg('age');                     // 同名 averge
\App\User::sum('salary');                  
```
...
whereDate('created_at', '2016-12-31')   // 库中 created_at 2019-09-09 09:09:09
...
// select * from `users` where `active` = 0 and `id` > 4 order by `date` asc, `id` asc limit 3
```
